...
public function getInscritosAceptadosEvento($idEvento) {
    ...
    // 3. Guardar asistencia y nota de un inscrito
public function guardarAsistenciaNota($idInscripcion, $asistencia, $nota) {
    ...
    if ($stmt->fetchColumn() > 0) {
        ...
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$asistencia, $nota, $insc['SECUENCIALEVENTO'], $insc['SECUENCIALUSUARIO']]);
    } else {
        // INSERT
        $sql = "INSERT INTO asistencia_nota (SECUENCIALEVENTO, SECUENCIALUSUARIO, ASISTIO, NOTAFINAL) VALUES (?, ?, ?, ?)";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$insc['SECUENCIALEVENTO'], $insc['SECUENCIALUSUARIO'], $asistencia, $nota]);
    }
}
}
